...
const PvPGame = () => {
  ...
  if (gameData?.currentQuestionIndex >= 10) {
    ...
    const winner = p1.score > p2.score ? "PLAYER 1 DOMINATED!" : (p2.score > p1.score ? "PLAYER 2 DOMINATED!" : "IT'S A TIE!");
      ...
      <div className="h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center p-6 text-center font-sans">
        <div className="relative group p-1">
          ...
          <div className="relative bg-[#16181a] border border-gray-800 p-10 rounded-3xl shadow-2xl">
            ...
            <button onClick={() => window.location.reload()} className="w-full bg-white text-black py-4 rounded-xl font-black hover:bg-gray-200 transition-all active:scale-95 uppercase tracking-widest">
              Play Again
            </button>
          </div>
        </div>
    ...
    return (
      <div className="h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center p-6 font-sans">
        ...
        <div className="w-full max-w-md space-y-6">
          <button 
            onClick={createRoom} 
            className="w-full relative group h-[120px] transition-all duration-300 active:scale-95"
          >
          ...
          <div className="relative">
            <input 
              ...
              className="w-full bg-black border-2 border-gray-800 p-5 rounded-2xl text-center font-black text-xl text-white focus:border-emerald-500 outline-none transition-all placeholder:text-gray-700"
            />
          </div>

          <button 
            onClick={joinRoom} 
            className="w-full h-[80px] bg-emerald-600 rounded-2xl font-black text-xl hover:bg-emerald-500 shadow-lg shadow-emerald-900/20 active:scale-95 transition-all uppercase"
          >
            Join Arena
          </button>
        </div>
  ...
  <div className="h-screen bg-[#0a0a0a] text-white p-4 flex flex-col font-sans overflow-hidden">
    ...
    <div className="flex-none flex justify-between items-center bg-[#111] border border-gray-800 p-4 rounded-2xl mb-4 shadow-xl">
      ...
      <div className="text-center">
        <span className="bg-gray-800 px-3 py-1 rounded-full text-xs font-bold">
          {(gameData?.currentQuestionIndex || 0) + 1}/10
        </span>
      </div>
    ...
    <div className={`flex-none text-center py-2 rounded-xl font-black mb-4 border-b-2 transition-all ${
      isMyTurn ? 'bg-emerald-900/20 border-emerald-500 text-emerald-400' : 'bg-gray-900/50 border-gray-800 text-gray-600'
    }`}>
      ...
      <div className="flex-grow overflow-y-auto custom-scrollbar pb-6">
        <div className="max-w-2xl mx-auto w-full">
          <div className="relative bg-[#16181a] border border-gray-800 p-6 md:p-10 rounded-3xl shadow-2xl">
            ...
            <h3 className="text-xl md:text-3xl font-bold text-center mb-8 leading-tight text-gray-100" 
            ...
            <div className="grid grid-cols-1 gap-3">
              {currentQuestion.options.map((opt, i) => (
                ...
                >
                  <span className={`w-8 h-8 rounded-lg flex items-center justify-center mr-4 text-xs font-bold ${isMyTurn ? 'bg-gray-800 text-cyan-400' : 'bg-gray-900'}`}>
